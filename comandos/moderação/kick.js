...
exports.run = async (client, message, args) => {
  ...
  let erro = new Discord.MessageEmbed()
    ...
    .setColor("#2f3136")
    .setDescription(
      `<:pasta:727531432427388999> Informação deste erro: \n\`O comando de kick é utilizado para expulsar usuarios fazendo eles entrar mais em seu servidor! Basta usar ${prefix}kick <@menção> <motivo>\` \n\n<:moderation_hammer:727243867711209582> Permisão do comando: \n\`KICK_MEMBERS\``
    );
  ...
  if (membro === message.member)
    return message.reply(
      `<:close:735574187204280360> | Eu não posso autoexpulsar você mesmo!`
    );

  let motivo = args.slice(1).join(" ");
  if (!motivo) return message.channel.send(erro);

  if (!message.member.hasPermission("KICK_MEMBERS")) return message.reply(erro);

  let banido = new Discord.MessageEmbed()
    .setTitle(`<:ban:735582739545456731> Expulsamento!`)
    .setColor("#2f3136")
    .setDescription(
